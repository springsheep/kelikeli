<template>
  <div class="content">
    ...
    <div style="min-height: 94%">
      <div class="title" style="line-height: 1.6rem;font-size: .64rem;color: #666666">绑定车辆</div>
      <div class="input">
        <div @click="getItem1">
          <el-input placeholder="请输入车牌号" v-model="input1" readonly="readonly" style=""></el-input>
        </div>
        <el-input placeholder="请输入手机号" v-model="input2" style="margin-top: .54rem" pattern="[0-9]*"></el-input>
        <div style="position:relative;">
          <el-input placeholder="请选择车型品牌" v-model="input3" style="margin-top: .64rem;"></el-input>
          <div style="position: absolute;right: 0;padding-right: .6rem;bottom: .55rem;width: 2rem;text-align: right"  @click="choosecar">
            <img src="../../assets/32.png" alt=""  style="width: .3rem">
          </div>
        </div>
        <!--<el-input placeholder="请选择车身颜色" v-model="input4" style="margin-top: .64rem" v-if="popupVisible" ></el-input>-->
        <!--<div style="position:relative;">-->
          <!--<el-input placeholder="请选择车身颜色" v-model="input4" style="margin-top: .64rem;"-->
                    <!--readonly="readonly"></el-input>-->
          <!--<div style="position: absolute;right: .6rem;bottom: .55rem;width: 2rem;text-align: right"  @click="popupVisible=true">-->
            <!--<img src="../../assets/32.png" alt=""  style="width: .3rem">-->
          <!--</div>-->
        <!--</div>-->
    ...
    <div class="mask" @click="close" v-if="ifshowtip">
      <img src="../../assets/help.png" width="93.4%"/>
    </div>
    <!--选择车牌号 首个汉字键盘 弹出层-->
...
<script>
  ...
  export default {
    ...
    watch: {
      input1() {
        this.$store.state.bindcarinput1 = this.input1
      },
      input2() {
        this.$store.state.bindcarinput2 = this.input2
      }
    }
    ,
    created() {
      ...
      if (this.$store.state.bindcarinput1 == null) {
      } else {
        this.input1 = this.$store.state.bindcarinput1
      }
      if (this.$store.state.bindcarinput2 == null) {
      } else {
        this.input2 = this.$store.state.bindcarinput2
      }
    },
    methods: {
      choosecar() {
      ...
      next() {
        ...
        if ((!/^[京津沪渝冀豫云辽黑湘皖鲁新苏浙赣鄂桂甘晋蒙陕吉闽贵粤青藏川宁琼使领A-Z]{1}[A-Z]{1}[A-Z0-9]{4}[A-Z0-9挂学警港澳]{1}$/.test(this.input1))) {
          Toast('请填写正确的车牌');
          return false;
          ...
          this.axios.post('moveCar/bindMoveCarQrCode', {
            ...
            if (res.data.state == 1) {
              Toast('绑定成功');
              this.$router.push({path: '/akaconcernWx'})
            } else {
