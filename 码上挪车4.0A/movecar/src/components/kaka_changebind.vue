<template>
  <div class="content">
    ...
    <van-overlay
      :show="$store.state.changebindstate"
    />
      ...
      <div class="changecontent ">
        <div @click="getItem1">
          <mt-field placeholder="请输入新车牌" style="width: 93.3%;margin-left: 3.3%;margin-top: .5rem;" v-model="input4"
                    readonly="readonly" :disableClear="true">
          </mt-field>
        </div>
        <div style="position: relative">
          <mt-field placeholder="请选择车型品牌" :disableClear="true" style="width: 93.3%;margin-left: 3.3%;margin-top: .5rem;"
                    v-model="input5"></mt-field>
    ...
    <div class="changephone1 bounceIn animated" v-if="$store.state.changebindstate">
      ...
      <div class="changecontent ">
        ...
        <mt-field placeholder="请输入手机号码" type="number"
                  style="width: 93.3%;margin-left: 3.3%;margin-top: .7rem;color: #999999"
                  v-model="input2">
        </mt-field>
        <mt-field placeholder="请输入验证码" style="width: 93.3%;margin-left: 3.3%;margin-top: .7rem;color: #999999"
                  v-model="input3" type="number">
    ...
    <div class="top">
      <div>
        <p style="font-size: .8rem">尊敬的<span style="font-size: .788rem;color: #04c8af">{{carNo}}</span>车主</p>
    ...
    >
      ...
      <img src="../assets/163.png"/>
    </div>
    <div class="foot">
      <!--点击进入挪车码申请-->
      <div class="ppp" @touchstart.prevent="movestart(1)" @touchend.prevent="moveend(1)" ref="ppp">
        <span><img src="../assets/167.png"/></span>
        <p>挪车码申请</p>
      </div>
      <!--点击进入我的挪车码-->
      <!--<div class="ppp" @touchstart.prevent="movestart(5)" @touchend.prevent="moveend(5)" ref="ppp">-->
      ...
      <div class="second"
           ...
           ref="ppp1"
      >
        <span style="padding-top: .65rem;"><img src="../assets/123.png"/></span>
        <p>低价购车险</p>
      </div>
      <div class="three"
           ...
           ref="ppp2"
      >
        <span style="padding-top: .7rem;"><img src="../assets/121.png"/>
        </span>
        <p>更多服务项</p></div>
    </div>
    ...
    methods: {
      ...
      sure() {
        if (!(/^1[3456789]\d{9}$/.test(this.input2))) {
          ...
          this.axios.post('moveCar/checkBindMobilePhoneNo', {
            "qrCode": sessionStorage.getItem('qrcode'),
            "openId": this.openId,
      ...
      //点击完成按钮 完成绑定
      Complete() {
        if ((!/^[京津沪渝冀豫云辽黑湘皖鲁新苏浙赣鄂桂甘晋蒙陕吉闽贵粤青藏川宁琼使领A-Z]{1}[A-Z]{1}[A-Z0-9]{4}[A-Z0-9挂学警港澳]{1}$/.test(this.input4))) {
          Toast('请填写正确的车牌');
          return false;
        } else if (this.input5 == '') {
          Toast('请填写车辆品牌');
      ...
      getcode() {
        if (this.input2 == '') {
          ...
          if (this.canClick) {
            this.axios.post('moveCar/getCaptcha', {
              "qrCode": sessionStorage.getItem('qrcode'),
              "openId": this.openId,
              "mobilePhoneNo": this.input2
            }).then((res) => {
