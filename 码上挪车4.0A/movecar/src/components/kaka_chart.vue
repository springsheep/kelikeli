...
align-items: center;justify-content: center" @click="showinput=true">
      ...
      <input type="text"
             style=" font-size: .64rem;padding-left: .4rem;border-radius: 2px;margin-left: .64rem;border: 0;background: #ffffff;height: 1.6rem;width: 200%"
...
<script>
  ...
  export default {
    ...
    beforeRouteLeave(to, from, next) {
      Dialog.confirm({
        title: '温馨提示',
        message: '若您离开或者关闭此页面，将无法接收到回复消息',
        cancelButtonText: '离开',
    ...
    data() {
      return {
        ...
        testContents: [],
        content: this.$store.state.choosename,
        //聊天记录
        records: [],
        imgFile: {},
        EXPS: [],
    ...
    watch: {
      ...
      //     let obj = {type: '1', content: this.$store.state.choosename, time: ''}
    ...
    methods: {
      ...
      //播放语音
      play(m, c) {
        console.log(`${m.content.split(`${this.uuuu}`)[0]}` ,11111)
        this.playnum = c
        var _this = this;
      ...
      initWebSocket() { //初始化weosocket
        console.log('开始连接');
        var wsuri = `wss://${wx1.websocket}/im/getMessage`;
      ...
      websocketonopen() { //连接建立之后执行send方法发送数据
        let obj = {type: '0', content: `${this.qrcode},${this.openId}`}
        this.websock.send(JSON.stringify(obj));
        console.log(obj)
        this.successtype = 1
      },
                  ...
                  wx.uploadVoice(
                    {
                      ...
                      success: function (res2) {
                        console.log(res2.serverId, '1234678')
                        vm.records.push({
                          time: new Date().getHours()+':'+new Date().getMinutes(),
                          content: duration + '"',
