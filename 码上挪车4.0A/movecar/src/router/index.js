...
import kaka_home from '../components/kaka_home'//完成绑定
...
import kaka_changebind from '../components/kaka_changebind'//修改绑定号码
...
const router = new Router({
  routes: [
    ...
    {path: '/index/:qrCode', name: 'index', component: Index},

    {path: '/kaka_loading', component: kaka_loading, },
    {path: '/akaconcernWx', component: kaka_concernWx, meta: {index: 0.3, title: ''}},
    {path: '/kaka_bindCar', component: kaka_bindCar, meta: {index: 0.2},},
    {path: '/kaka_home', component: kaka_home, meta: {index: 1, title: '码上挪车'}},
    ...
    {path: '/kaka_home1', component: kaka_home1, meta: {index: 0.2},},
    ...
    {path: '/kaka_changebind', component: kaka_changebind, meta: {index: 1, title: '更改绑定'}},
...
router.beforeEach((to, from, next) => {
  ...
  if (to.path == '/index') {
    ...
    if (!code) {
      ...
      axios.post('/moveCar/moveCarQrCodeInit', {
        ...
        if (res.data.state == '1') {
          ...
          if (res.data.data.isBind === '0') {
            next('/kaka_bindCar')
          } else if (res.data.data.isBind === '1') {
  ...
  if (to.name == 'index') {
    sessionStorage.setItem('qrcode', to.params.qrCode)
    store.state.qrcode = to.params.qrCode
    localStorage.setItem('qrcode', to.params.qrCode)
  }

  if (sessionStorage.getItem('openId') != undefined) {
    var openid = sessionStorage.getItem('openId');
  }
  if(to.path=='/kaka_loading'){
    let code = getUrlKey("code");
    if (!code) {
      getCodeApi();
    } else {
      axios.post('/moveCar/moveCarQrCodeInit', {
        ...
        if (res.data.state == '1') {
          ...
          if (res.data.data.isBind === '0') {
            ...
            if (res.data.data.isCarOwner === '1') {
              next('/kaka_changebind')
            } else {
              next('/kaka_home')
            }
          }
